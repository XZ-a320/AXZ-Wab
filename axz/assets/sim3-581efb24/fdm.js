/* ==========================================================================
   AXZ sim 2.0 — flight dynamics.

   The 1.0 model, kept whole, plus the things a 1.0 pilot asked for: flaps
   that TRAVEL rather than snap, spoilers that deploy themselves on
   touchdown, reversers, fuel that burns and lightens the aeroplane, a wet
   runway that is slippery, the buffet before a stall, and an altitude and
   heading hold. Every 1.0 behaviour the gates check is still here.
...
import {
  clamp, DEG, RAD, approach, qrot, qinv, qmul, qnorm, qFromAxisAngle, qFromEuler, qToEuler,
...
/* Fuel on board at a scenario start, as a fraction of the published mass.
   Not a tank table: the point is that the aeroplane gets LIGHTER, so a long
   flight lands slower than it took off, which is true of every real one. */
function fuelFor(spec, cfg) {
  if (cfg.prop) return cfg.mass * 0.09
  if (cfg.engine === 'turbofan-ab' || cfg.engine === 'turbojet-reheat') return cfg.mass * 0.28
  return cfg.mass * 0.22
}
/* Specific fuel consumption, kg per newton-second. Real orders of magnitude:
   a high-bypass fan burns about 0.6 lb per lbf per hour, a reheated turbojet
   three times that with the burner lit. */
const SFC = {
  turbofan: [1.7e-5, 1.7e-5], 'turbofan-small': [1.9e-5, 1.9e-5],
  'turbofan-ab': [2.3e-5, 5.5e-5], 'turbojet-reheat': [2.6e-5, 5.0e-5], piston: [0, 0],
}
const linearTo = (from, to, maxStep) => (to > from ? Math.min(to, from + maxStep) : Math.max(to, from - maxStep))
...
export class Aircraft {
  constructor(spec, contacts, restHeight) {
    ...
    /* --- 2.0 --------------------------------------------------------------- */
    this.flapPos = 0              // where the flaps actually are, in detents
    this.spoilerPos = 0           // where the panels actually are, 0..1
    this.autoSpoiler = false      // deployed by the touchdown, not the pilot
    this.autoSpoilerArm = false
    this.reverse = false          // reversers selected
    this.reversePos = 0           // and where they actually are
    this.lights = false           // landing lights
    this.buffet = 0               // 0..1, the airframe shaking before the stall
    this.surfaceWet = 0           // 0..1, from the weather
    this.ap = { on: false, alt: 0, hdg: 0 }
    this.fuel0 = fuelFor(spec, this.cfg)
    this.fuel = this.fuel0
    this.burnRate = 0             // kg/s, for the readout
  ...
  /** Mass right now: the published figure less the fuel already burned. */
  get mass() { return this.cfg.mass - this.fuel0 + this.fuel }
  /** Can this type reverse its engines? Fans and Concorde's Olympus can; a burner cannot. */
  get hasReverse() { return ['turbofan', 'turbofan-small', 'turbojet-reheat'].includes(this.cfg.engine) }
  /** Does this type arm its spoilers for the touchdown? Transports do. */
  get hasAutoSpoiler() { return ['airliner', 'airbus', 'bizjet'].includes(this.spec.flapSet) }
  /** The flap setting the wing is actually at, interpolated between detents while travelling. */
  flapNow() {
    const f = this.flaps, p = clamp(this.flapPos, 0, f.length - 1)
    const i = Math.floor(p), t = p - i
    if (t < 1e-3 || i >= f.length - 1) return f[i]
    const a = f[i], b = f[i + 1]
    return {
      deg: a.deg + (b.deg - a.deg) * t, dCL: a.dCL + (b.dCL - a.dCL) * t, dCD: a.dCD + (b.dCD - a.dCD) * t,
      dStall: a.dStall + (b.dStall - a.dStall) * t, vfe: Math.min(a.vfe, b.vfe), label: b.label != null ? b.label : a.label,
    }
  }
  get flapMoving() { return Math.abs(this.flapPos - this.flap) > 0.01 }
  ...
  place(x, y, z, headingDeg, speedMs, { onGround = false, gamma = 0, trimmed = false, wind = null } = {}) {
    ...
    // A placement is a new flight: surfaces where the lever says, tanks full.
    this.flapPos = this.flap
    this.spoilerPos = this.spoilers
    this.autoSpoiler = false; this.autoSpoilerArm = false
    this.reverse = false; this.reversePos = 0
    this.ap.on = false
    this.fuel = this.fuel0
    this.buffet = 0
  }
  ...
  step(dt) {
    const cfg = this.cfg
    /* Flaps TRAVEL. A 737's go from up to 30 in about twenty seconds, and the
       lift arrives as they get there, not when the lever moves — which is why
       selecting landing flap late is a thing you feel for a while. */
    const hydOk = this.sys.hyd > 0 && this.sys.flap > 0
    if (hydOk) {
      const detentsPerSec = cfg.prop ? 0.55 : this.flaps.length > 3 ? 0.16 : 0.40
      this.flapPos = linearTo(this.flapPos, this.flap, detentsPerSec * dt)
    }
    const flap = this.flapNow()
    this.spoilerPos = linearTo(this.spoilerPos, this.spoilers > 0.5 ? 1 : 0, 0.8 * dt)
    /* Reversers: only on the ground, only on a type that has them, and they
       stow themselves the moment the wheels leave. */
    if (!this.onGround || !this.hasReverse) this.reverse = false
    this.reversePos = linearTo(this.reversePos, this.reverse ? 1 : 0, 0.9 * dt)

    /* Altitude and heading hold. Not a flight director: it flies pitch to a
       vertical-speed demand and bank to a heading demand, with rate damping,
       the way a basic autopilot does. The pilot's own inputs take it out
       (the loop in main.js does that), so it cannot fight a hand on the yoke. */
    if (this.ap.on && !this.crashed && !this.onGround) {
      const e = qToEuler(this.q)
      const vsCmd = clamp((this.ap.alt - this.pos.y) * 0.12, -9, 9)
      const pitchCmd = clamp(e.pitch + (vsCmd - this.vel.y) * 0.025, -10 * DEG, 14 * DEG)
      this.ctl.elevator = clamp((pitchCmd - e.pitch) * 3.2 - this.omega.x * 1.4, -0.6, 0.6) - this.trim
      let hdgErr = ((this.ap.hdg - (e.heading * RAD + 360) % 360 + 540) % 360) - 180
      const bankCmd = clamp(hdgErr * 0.7, -25, 25) * DEG
      this.ctl.aileron = clamp((bankCmd - e.bank) * 1.4 + this.omega.z * 0.7, -0.5, 0.5)
      this.ctl.rudder = 0
    }
    ...
    let CL = this.liftCoef(this.alpha, flap.dCL, stallAlpha) + this.spoilerPos * -0.55
    ...
    const CD0 = cfg.cd0 + flap.dCD + this.gearPos * 0.019 + this.spoilerPos * 0.055
    const kInd = 1 / (Math.PI * cfg.oswald * cfg.AR)
    const CD = CD0 + waveDrag(cfg, mach) + kInd * CL * CL * ge
    const CY = -0.90 * this.beta

    this.stalling = Math.abs(this.alpha) > stallAlpha && V > 12
    /* Buffet: the airframe shakes before the wing lets go, and again through
       the transonic. Read by the camera and the sound, not by the physics. */
    const margin = stallAlpha - Math.abs(this.alpha)
    let buffet = V > 30 && !this.onGround ? clamp(1 - margin / (3 * DEG), 0, 1) : 0
    if (mach > cfg.mdd && mach < 1.05) buffet = Math.max(buffet, 0.6 * clamp((mach - cfg.mdd) / (1.05 - cfg.mdd), 0, 1) * (1 - clamp((mach - 1.0) / 0.05, 0, 1)))
    this.buffet = buffet
    ...
    const gross = this.crashed ? 0
      : this.thrustAvailable(rho, V, this.thrustLag) * this.engineFraction
    // A reverser turns about half the fan air forward. Net thrust goes
    // negative, which is what stops a 65-tonne aeroplane on a wet runway.
    const thrust = gross * (1 - 1.5 * this.reversePos)
    this.netThrust = thrust
    Fb = vadd(Fb, { x: 0, y: 0, z: -thrust })
    /* Fuel. Burned at the engines' specific consumption for what they are
       actually producing, wet or dry, and it comes straight off the mass. */
    if (!this.crashed) {
      const sfc = SFC[cfg.engine] || SFC.turbofan
      const ab = this.abFrac
      this.burnRate = cfg.prop ? 0.0085 * this.thrustLag * this.engineFraction
        : gross * (sfc[0] + (sfc[1] - sfc[0]) * ab)
      this.fuel = Math.max(0, this.fuel - this.burnRate * dt)
      if (this.fuel <= 0 && this.engineFraction > 0) {
        for (let i = 0; i < this.eng.length; i++) this.eng[i] = 0
        this.note('fuel', 'exhausted')
      }
    }
    ...
    let F = vadd(Faero, { x: 0, y: -this.mass * G, z: 0 })
    ...
    this.gLoad = (vdot(vadd(Faero, gr.force), bodyUp) / this.mass) / G

    /* Ground spoilers. Armed on the approach, deployed by the wheels taking
       weight with the levers at idle, and stowed again if the levers come up
       for a go-around or a take-off roll. */
    if (this.hasAutoSpoiler) {
      if (!this.onGround && this.gearPos > 0.9 && this.radioAlt < 400) this.autoSpoilerArm = true
      if (this.autoSpoilerArm && this.onGround && this.tas > 25 && this.throttle < 0.35) {
        this.spoilers = 1; this.autoSpoiler = true; this.autoSpoilerArm = false
      }
      if (this.autoSpoiler && (this.throttle > 0.5 || !this.onGround)) { this.spoilers = 0; this.autoSpoiler = false }
    }

    // --- Integrate (semi-implicit Euler; stable at the 240 Hz this runs at).
    // Inertias scale with the mass on board, so a light aeroplane is livelier.
    const mScale = this.mass / cfg.mass
    const acc = vscale(F, 1 / this.mass)
    this.vel = vadd(this.vel, vscale(acc, dt))
    this.pos = vadd(this.pos, vscale(this.vel, dt))

    const alphaDot = {
      x: (M.x - (cfg.Iz - cfg.Iy) * mScale * this.omega.y * this.omega.z) / (cfg.Ix * mScale),
      y: (M.y - (cfg.Ix - cfg.Iz) * mScale * this.omega.z * this.omega.x) / (cfg.Iy * mScale),
      z: (M.z - (cfg.Iy - cfg.Ix) * mScale * this.omega.x * this.omega.y) / (cfg.Iz * mScale),
    }
  ...
  aeroMoments(qbar, V, flap, dtHint = 1 / 240) {
    ...
    // In ground effect the downwash at the tail weakens and the nose wants to
    // drop a little, which is part of why the flare needs a steady pull.
    const hbGE = clamp(this.agl / cfg.b, 0, 1)
    if (!this.onGround) Cm -= 0.018 * Math.exp(-4 * hbGE)
  ...
  groundForces(dt) {
    ...
    for (const c of this.contacts) {
      ...
      const wet = this.surfaceWet
      const muRoll = c.tail ? 0.62 : 0.02 + this.brakes * 0.55 * (1 - 0.6 * wet) + (this.parkingBrake ? 0.9 * (1 - 0.4 * wet) : 0)
      /* Side grip. At 0.85 with a viscous term stiff enough to saturate it at
         ...
         near even that. */
      const muSide = c.tail ? 0.62 : 0.55 * (1 - 0.45 * wet)
      ...
      let fF = clamp(-vF * this.mass * 0.9, -muRoll * N, muRoll * N)
      let fS = clamp(-vS * this.mass * 0.9, -muSide * N, muSide * N)
    ...
    if (wheelContact && this.wasAirborne) {
      ...
      this.justLanded = {
        ...
        bankDeg: Math.abs(e.bank) * RAD,
        pitchDeg: e.pitch * RAD,
        vrefKt: this.vrefMs() * 1.943844,
      }
  ...
  get flapDeg() { return this.flapNow().deg }
  /** What the detent is MARKED, which is not always a number of degrees. */
  get flapLabel() {
    const f = this.flaps[this.flap]
    const l = f.label != null ? f.label : String(f.deg)
    return this.flapMoving ? l + '›' : l
  }
  /** Reversers, if the type has them and the wheels are down. */
  toggleReverse() {
    if (!this.hasReverse || !this.onGround) return false
    this.reverse = !this.reverse
    return true
  }
  /** Hold the current altitude and heading. */
  engageAutopilot() {
    if (this.onGround || this.crashed) return false
    const e = qToEuler(this.q)
    this.ap = { on: true, alt: this.pos.y, hdg: (e.heading * RAD + 360) % 360 }
    return true
  }
  disengageAutopilot() { this.ap.on = false }
  ...
  stallSpeedKt() {
    ...
    const v = Math.sqrt((2 * this.mass * G) / (1.225 * cfg.S * Math.max(CLmax, 0.3)))
    return v * 1.943844
  }
}
